...
;(function(){
  ...
  function baliseAchats(){ var v=document.getElementById('view-achats'); if(!v) return; var headHas=!!v.querySelector('.view-head .ep-ach-kpis'); var cards=[].slice.call(v.querySelectorAll('[style*="min-width:96px"]')).filter(function(c){ return !c.closest('.view-head'); }); var conts=[]; cards.forEach(function(c){ var pa=c.parentElement; if(pa && conts.indexOf(pa)<0) conts.push(pa); }); conts.forEach(function(cont){ if(cont.classList.contains('ep-ach-kpis')) return; if(headHas){ cont.style.setProperty('display','none','important'); } else { cont.classList.add('ep-ach-kpis'); headHas=true; } }); }
  function place(){
...
;(function(){
  ...
  function norm(){ var now=Date.now(); if(now-_t<150) return; _t=now;
    ...
    if(!v.querySelector('.crumb')){
      ...
      var crumb=cand[0];
      if(crumb && crumb.childNodes.length<=1){ try{ var _I=window.EPilotI18n, _lang=window.__appLang||"fr"; if(_I){ var _now=crumb.textContent.replace(/\s+/g," ").trim(); if(/\//.test(_now)){ var _parts=_now.split("/").map(function(s){return s.trim();}); var _isEn=_parts.some(function(p){return _I.toFR[p]!==undefined;}); if(_isEn) crumb.setAttribute("data-ep-co",_now); var _orig=crumb.getAttribute("data-ep-co"); if(_orig){ var _tr; if(_lang==="en"){ _tr=_orig; } else { _tr=_orig.split("/").map(function(x){x=x.trim();return _I.toFR[x]||x;}).join(" / "); } if(_now!==_tr) crumb.textContent=_tr; } } } }catch(e){} }
